...
void setup() {
  ...
  pinMode(LATCH_BUTTON, INPUT); //pin4
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              if (digitalRead(SECURELOCK) == LOW){
                ...
                Serial.println("Secure lock off!\nWaiting for user to press door button");
                client.send("Secure lock off. Push button!");
              }
              //wait for user to press button
              //With timeout sub-feature
              unsigned long waitingTime = 30000; //90000 = 90-sec (1.5-min) waiting time until it times out
              unsigned long startWaitTime = millis();
              unsigned long currWaitTime = 0;
              while((lockButton == LOW) && (currWaitTime < waitingTime)){ 
                currWaitTime = millis()-startWaitTime;
              ...
              if(currWaitTime >= waitingTime){
                digitalWrite(SECURELOCK, LOW);
                Serial.println("Timed out. Secure lock enabled.");
              ...
              /*while(lockButton == LOW){ lockButton = digitalRead(LATCH_BUTTON); } //without timeout
              //while(lockButton == HIGH){ lockButton = digitalRead(LATCH_BUTTON); } //without timeout
              open_door(client);
              //If it has reached X secs, lock the latch again. (X = 2 seconds)
              while(millis() - interval <= door_opened_millis) {} // current time - face recognised time > X secs
              digitalWrite(SECURELOCK, LOW); //open relay              
