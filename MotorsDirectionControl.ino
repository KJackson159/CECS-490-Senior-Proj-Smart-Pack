...
uint8_t broadcastAddress[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}; //UPDATE THIS WITH ADDRESS OF "MCU A"
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  Serial.println(len);
  motorsState = myData.myState;
  modeON = myData.running;
}
...
void setup() {
  ...
  motorsState = "stop"; //initialize state to STOP
  Serial.begin(115200);
